...
def _get_cell_value(df, ref):
    """Return the value at *ref* in *df* if possible."""
    if ref is None:
        return None
...
class EditableHeaderView(QHeaderView):
    ...
    def editHeader(self, section):
        ...
        new_text, ok = QInputDialog.getText(
            self, "Edit Header", "Enter new header text:", text=current_text
        )
...
class ExcelViewer(QWidget):
    ...
    def init_ui(self):
        ...
        self.horizontal_header = EditableHeaderView(
    ...
    def create_toolbar(self, main_layout):
        ...
        self.scope_selector.setToolTip(
            "Choose whether to apply operations to the current sheet or all sheets"
        )
        ...
        header_rows_str = " and ".join(
            [str(r + 1) for r in self.report_config["header_rows"]]
        )
        ...
        self.rename_columns_button.setToolTip(
            "Import column headers from SQL query results"
        )
        ...
        self.remove_empty_rows_button.setToolTip(
            "Remove rows that have no data values (preserves first 2 columns)"
        )
        self.remove_empty_rows_button.clicked.connect(self.remove_empty_data_rows)

        # Extract SQL button
        self.extract_sql_button = QPushButton("Extract SQL")
        self.extract_sql_button.setToolTip(
            "Extract center codes and account numbers for SQL IN clauses"
        )
    ...
    def apply_filter(self):
        ...
        filter_info = (
            f" (filtered by {filter_col}: '{filter_text}')" if filter_text else ""
        )

        if self.sheet_name:
            self.status_label.setText(
                f"Sheet: {self.sheet_name} | {rows} rows, {cols} columns{filter_info}"
            )
        else:
    ...
    def clear_filter(self):
        ...
        if self.filtered_df is not None:
            ...
            if self.sheet_name:
                self.status_label.setText(
                    f"Sheet: {self.sheet_name} | {rows} rows, {cols} columns"
                )
            else:
                self.status_label.setText(f"{rows} rows, {cols} columns")
    ...
    def is_sheet_already_cleaned(self):
        ...
        if (
            len(self.df) < min_required_rows
        ):  # Not enough rows to contain the original headers
            return True
    ...
    def _import_column_headers_to_sheet(self):
        ...
        if (
            ...
            and hasattr(parent, "results_viewer")
        ):
            ...
            if sql_df is None or sql_df.empty:
                ...
                QMessageBox.warning(
                    self,
                    "No SQL Data",
                    "Please run a SQL query first to get column headers.",
                )
                return
            ...
            apply_all_button.clicked.connect(
                lambda: self._apply_sql_headers(sql_columns, mapping_dialog)
            )
            apply_selected_button.clicked.connect(
                lambda: self._apply_sql_headers(
                    [
                        str(column_list.item(idx).text())
                        for idx in range(column_list.count())
    ...
    def _import_column_headers_to_all_sheets(self):
        ...
        if not hasattr(parent, "results_viewer") or not hasattr(
            parent, "excel_analyzer"
        ):
            QMessageBox.warning(
                self, "Error", "Could not access the SQL results or Excel analyzer."
            )
            return
        ...
        cs_layout.addWidget(
            QLabel("Select SQL columns to import as headers to all sheets:")
        )
        ...
        if (
            column_selection_dialog.exec() != QDialog.DialogCode.Accepted
        ...
        layout = QVBoxLayout(mapping_dialog)
        layout.addWidget(
            QLabel("How would you like to apply the SQL headers to all sheets?")
        )
        ...
        position_btn = QRadioButton(
            "Map by position (SQL column 1 → Excel column 1, etc.)"
        )
        ...
        preserve_btn = QRadioButton(
            "Preserve Sheet_Name column if present (SQL column 1 → Excel column 2, etc.)"
        )
        ...
        progress = QProgressDialog(
            "Importing headers...", "Cancel", 0, len(sheet_names), self
        )
        ...
        try:
            ...
            for i, sheet_name in enumerate(sheet_names):
                ...
                if mapping_method == 1:  # Position mapping
                    ...
                    has_sheet_name = (
                        "Sheet_Name" in new_columns and new_columns[0] == "Sheet_Name"
                    )
    ...
    def _base_clean_dataframe(self, df, sheet_name):
        ...
        try:
            ...
            center_cell = self.report_config.get("center_cell")
            if center_cell:
                val = _get_cell_value(df, center_cell)
                if val is not None and str(val).strip():
                    sheet_name = str(val).strip()
            ...
            for i in range(data_df.shape[1]):
                ...
                is_empty = (
                    col_data.isna().all()
                    or (col_data.astype(str).str.strip() == "").all()
                )
            ...
            columns_to_keep = [
                i for i in range(data_df.shape[1]) if i not in empty_cols
            ]
            ...
            if len(new_headers) == len(clean_df.columns):
                ...
                clean_df.columns = [
                    f"Column_{i+1}" for i in range(len(clean_df.columns))
                ]
                self.logger.info(
            ...
            if self.report_type == "Corp SOO":
                ...
                all_numeric_blank_or_zero = (blank_mask | numeric_zero_mask).all(axis=1)
    ...
    def _remove_empty_data_rows_from_sheet(self):
        ...
        try:
            ...
            empty_condition = self.df[data_columns].isna().all(axis=1) | (
                self.df[data_columns] == ""
            ).all(axis=1)
            ...
            if not all(rows_to_keep):
                ...
                if result == QMessageBox.StandardButton.Yes:
                    ...
                    if self.sheet_name:
                        self.status_label.setText(
                            f"Sheet: {self.sheet_name} | {rows} rows, {cols} columns | Removed {rows_to_remove} empty rows"
                        )
                    else:
                        self.status_label.setText(
                            f"{rows} rows, {cols} columns | Removed {rows_to_remove} empty rows"
                        )
            ...
            QMessageBox.critical(
                self, "Error Removing Empty Rows", f"An error occurred: {str(e)}"
            )
    ...
    def _remove_empty_data_rows_from_all_sheets(self):
        ...
        if (
            not parent
            or not isinstance(parent, MainWindow)
            or not hasattr(parent, "excel_analyzer")
        ):
            QMessageBox.warning(
                self,
                "Error",
                "Could not access the main application or Excel analyzer.",
            )
            return
        ...
        progress = QProgressDialog(
            "Removing empty rows...", "Cancel", 0, len(sheet_names), self
        )
        ...
        try:
            ...
            for i, sheet_name in enumerate(sheet_names):
                ...
                # Find rows where all data columns are NULL/NaN or empty string
                empty_condition = df[data_columns].isna().all(axis=1) | (
                    df[data_columns] == ""
                ).all(axis=1)
                rows_to_keep = ~empty_condition
            ...
            if rows_removed_count > 0:
                ...
                QMessageBox.information(
                    self, "Process Completed", "No empty rows found in any sheet."
                )
    ...
    def extract_sql_codes(self):
        ...
        try:
            ...
            if not parent or not isinstance(parent, MainWindow):
                ...
                QMessageBox.warning(
                    self, "Error", "Could not access the main application window."
                )
                return
            ...
            if not is_name_column and self.report_type in (
                "SOO MFR",
                "MFR PreClose",
                "Corp SOO",
            ):
            ...
            center_sql = (
                "IN (" + ", ".join([f"'{code}'" for code in sorted(center_codes)]) + ")"
            )
            account_sql = (
                "IN ("
                + ", ".join([f"'{code}'" for code in sorted(account_codes)])
                + ")"
            )

            # Create and show the result dialog
            self.show_extracted_sql(
                center_sql,
                account_sql,
                center_codes,
                account_codes,
    ...
    def show_extracted_sql(
        self,
        center_sql,
        account_sql,
        center_codes,
        account_codes,
        analyzed_sheets=None,
        missing_column_sheets=None,
    ):
        ...
        if missing_column_sheets and len(missing_column_sheets) > 0:
            warning_label = QLabel(
                f"Warning: The following sheets did not have the selected column: {', '.join(missing_column_sheets)}"
            )
        ...
        summary = QLabel(
            f"Found {len(center_codes)} center codes and {len(account_codes)} account codes"
        )
        ...
        if account_codes:
            ...
            # Add explanation
            explanation = QLabel(
                "Select accounts that should have their signs flipped during comparison (negative numbers become positive, positive numbers become negative):"
            )
            ...
            select_all_btn.clicked.connect(
                lambda: self._select_all_accounts(account_list)
            )
        ...
        insert_sql_btn.clicked.connect(
            lambda: self.insert_sql_to_editor(
                center_sql,
                account_sql,
    ...
    def clean_all_sheets(self):
        ...
        if not sheet_names:
            QMessageBox.warning(
                self, "No Sheets", "No Excel sheets are available to clean."
            )
            return
        ...
        progress = QProgressDialog(
            "Cleaning sheets...", "Cancel", 0, len(sheet_names), self
        )
        ...
        try:
            ...
            traceback.print_exc()
            QMessageBox.critical(
                self, "Error", f"An error occurred while cleaning sheets: {str(e)}"
            )
        finally:
            progress.close()
    ...
    def _clean_dataframe(self, df, sheet_name):
        ...
        if self.report_type in ("SOO MFR", "MFR PreClose"):
            ...
            if not hasattr(self, "_mfr_month_year"):
                ...
                mon, ok1 = QInputDialog.getItem(
                    self, "Month", "Select Month", months, 0, False
                )
                years = [str(y) for y in range(2000, 2101)]
                yr_text, ok2 = QInputDialog.getItem(
                    self, "Year", "Select Year", years, 0, False
                )
                if not (ok1 and ok2):
                    return clean_df
                try:
    ...
    def clean_sheet(self):
        ...
        try:
            ...
            header_rows_text = ", ".join(
                [str(idx + 1) for idx in self.report_config["header_rows"]]
            )
            ...
            skip_note = f"- {self.report_config['skip_rows']} rows above the header have been removed\n"
    ...
    def reset(self):
        ...
        self.model = PandasTableModel(empty_df, self.current_theme.lower() == "dark")
        self.table_view.setModel(self.model)
