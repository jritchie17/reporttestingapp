            ...
            if role == Qt.ItemDataRole.DisplayRole:
                ...
                try:
                    ...
                    try:
                        ...
                        value = list(self._data[row].values())[col] if col < len(self._data[row]) else None
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            if orientation == Qt.Orientation.Horizontal:
                try:
                    ...
                    if hasattr(self._columns, 'keys') and isinstance(self._columns, dict):
...
class ResultsViewer(QWidget):
    ...
    def load_results(self, data, columns=None):
        ...
        if not columns and data:
            ...
            try:
                ...
                self.columns = [f"Column_{i+1}" for i in range(len(data[0]) if data and hasattr(data[0], "__len__") else 0)]
        else:
        ...
        # Update status
        self.status_label.setText(f"{len(data)} rows, {len(self.columns)} columns returned")
    ...
    def apply_filter(self, filter_text):
        ...
        # Update status
        self.status_label.setText(f"{len(filtered_data)} of {len(self.results_data)} rows displayed")
    ...
    def export_results(self):
        ...
        try:
            ...
            if file_path.lower().endswith('.csv'):
                ...
                if not any(file_path.lower().endswith(ext) for ext in ['.csv', '.xlsx', '.json', '.txt']):
                    file_path += '.csv'
                df.to_csv(file_path, index=False)
