...
class ExcelAnalyzer:
    ...
    def analyze_sheet(self, sheet_name, header_rows=5):
        ...
        try:
            ...
            self.potential_queries[sheet_name] = self._generate_potential_queries(
                sheet_name
            )
    ...
    def _detect_data_area(self, df):
        ...
        if not sheet_name or sheet_name not in self.sheet_data:
            ...
            for i, row in df.iterrows():
                if (
                    row.isna().all()
                    or (row.astype(str).str.strip().str.len() == 0).all()
                ):
                    empty_rows.append(i)
        ...
        empty_rows = (
            self.sheet_data[sheet_name].get("structure", {}).get("empty_rows", [])
        )
        potential_headers = (
            self.sheet_data[sheet_name]
            .get("structure", {})
            .get("potential_headers", [])
        )
    ...
    def _detect_numerical_columns(self, df):
        ...
        for col in range(df.shape[1]):
            ...
            numeric_count = col_values.apply(
                lambda x: isinstance(x, (int, float)) and not pd.isna(x)
            ).sum()
            numeric_ratio = (
                numeric_count / len(col_values) if len(col_values) > 0 else 0
            )
    ...
    def clean_excel_data(self, df, sheet_name, report_config=None):
        """Clean Excel data by rounding numeric values and optionally add a center column."""
        try:
            cleaned_df = df.copy()

            center_cell = None
            if report_config:
                center_cell = report_config.get("center_cell")
            if center_cell:
                val = _get_cell_value(cleaned_df, center_cell)
                if val is not None and str(val).strip():
                    cleaned_df.insert(0, "Center", str(val).strip())
            ...
            if ca_col is None:
                ...
                self.logger.warning(
                    f"CAReportName column not found. Defaulting to first column: {ca_col}"
                )
